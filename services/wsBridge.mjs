// services/wsBridge.mjs
import { WebSocketServer, WebSocket } from "ws";
import fetch from "node-fetch";

// === CONFIG (tout en dur) ===
const SUPRA_API_KEY = "1a2b3c4d5e6f7a8b9c0d1e2f3a4b5c6d7e8f9a0b1c2d3e4f5a6b7c8d9e0f1a2"; // <-- remplace par ta clé
const REST_BASE = "https://prod-kline-rest.supra.com";
const WS_URL = "wss://prod-kline-ws.supra.com";

const RESOLUTION = 1;
const CHUNK_SIZE = 30;
const REFRESH_MS = 2 * 60 * 1000; // re-évalue horaires marchés toutes les 2 min
const MIN_GAP_MS = 250; // 4 req/s REST
...
function partsFromTZ(date, timeZone) {
  ...
  const WDmap = WD[parts.find(p=>p.type==="weekday")?.value] ?? 0;
  const hour = +parts.find(p=>p.type==="hour")?.value;
  const minute = +parts.find(p=>p.type==="minute")?.value;
  return { wd: WDmap, hour, minute };
}
...
function isForexLikeOpen(d = new Date()) {
  ...
  if (wd === 0) return hour >= 22;     // dim 22:00+
  if (wd >= 1 && wd <= 4) return true; // lun-jeu
  if (wd === 5) return hour < 23;      // ven < 23:00
  return false;
}
...
// Attache le WSS sur le server HTTP (même port, path /ws/prices)
export function attachPriceWSS(server) {
  ...
  console.log("✅ WSS monté sur /ws/prices");
  ...
  // Tick global (1s) : envoie le snapshot consolidé à tous les clients
  setInterval(() => {
    const payload = buildSnapshot();
    wss.clients.forEach((c) => { if (c.readyState === 1) { try { c.send(payload); } catch {} } });
  }, 1000);
...
// Lance le cycle de rebalance périodique
